...
const promotionControllers = require('./promotion.controller');
const {
  calculateTotalStock,
...
const createProduct = catchAsync(async (req, res) => {
  ...
  // console.log('compareAtPrice from request:', compareAtPrice);
...
const updateOrderStatus = catchAsync(async (req, res) => {
  ...
  // Nếu chỉ dùng findByPk(id) thì order.user sẽ bị null -> không gửi được mail
  const order = await Order.findByPk(id, {
    include: [
      {
        model: User,
        attributes: ['email', 'firstName', 'lastName'], // Chỉ lấy thông tin cần thiết
      },
    ],
  });
  ...
  if (order.User && order.User.email) {
    // Gọi hàm gửi mail (chạy ngầm, không bắt Admin phải đợi gửi xong mới phản hồi)
    emailService.sendOrderStatusUpdateEmail(order.User.email, {
        orderNumber: order.number,
        orderDate: order.createdAt,
        status,
    }).catch(err => {
        // Chỉ log lỗi ra console để debug, không làm lỗi request của Admin
        console.error('❌ Lỗi gửi email cập nhật trạng thái:', err.message);
    });
  } else {
      console.warn('⚠️ Không tìm thấy email khách hàng, bỏ qua gửi mail.');
  }
...
const getAllPromotions = catchAsync(async (req, res) => {
    const {
        ...
        search = '',
        status = 'all',
        sortBy = 'createdAt', // Sửa lỗi potential: thêm lại sortBy
        sortOrder = 'DESC',
    } = req.query;
    ...
    // Logic Filter Trạng Thái (Đã được thảo luận)
    if (status !== 'all') {
        const now = new Date();
        switch (status) {
            case 'active':
    ...
    // Thực hiện truy vấn (dùng findAndCountAll để lấy tổng số lượng)
    ...
    res.status(200).json({
    // ⚠️ QUAN TRỌNG: Frontend (RTK Query) mong đợi 'status' hoặc 'success'
    status: 'success', 
    data: {
        // ⬅️ BƯỚC 1: Cần có key 'promotions' để Frontend đọc mảng
        promotions: promotions, 
        
        // ⬅️ BƯỚC 2: Cần có key 'pagination' để hiển thị phân trang
        pagination: {
            currentPage: parseInt(page),
